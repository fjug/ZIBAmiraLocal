...
template <typename REAL>
	bool QPBO<REAL>::Improve(int N, int* order_array, int* fixed_nodes, bool verbose)
{
	int p, i_index;
	Node* i;
	Node* _i[2];
	...
	for (p=0; p<N; p++)
	{
		...
		if (verbose)
		{
			int changed_num = 0, unlabeled_num = 0;
			for (i=nodes[0]; i<node_last[0]; i++)
			{
				i->label = what_segment(i);
				if (i->label == what_segment(GetMate0(i))) unlabeled_num ++;
			...
			if (changed_num) printf("[%d %d %d] ", p, changed_num, unlabeled_num);
		}
	}
