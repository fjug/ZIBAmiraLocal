...
#include <string.h>
#include <assert.h>
#include "blockMinCost.h"
#include <graphmatching/api.h>
...
template <typename FlowType, typename CostType> class GRAPHMATCHING_API MinCost
{
public:
	typedef int NodeId;
	typedef int EdgeId;
...
template <typename FlowType, typename CostType> 
	inline typename FlowType MinCost<FlowType, CostType>::GetRCap(EdgeId e)
{
	Arc* a = &arcs[2*e];
	return a->r_cap;
}
...
template <typename FlowType, typename CostType> 
	inline typename FlowType MinCost<FlowType, CostType>::GetReverseRCap(EdgeId e)
{
	Arc* a = &arcs[2*e+1];
	return a->r_cap;
}
...
template <typename FlowType, typename CostType> 
	inline typename CostType MinCost<FlowType, CostType>::PriorityQueue::GetKey(Node* i)
{
