...
double GraphMatching::LocalCallbackFn(SubgradientDecomposition::Subproblem& s, double* theta, double* g)
{
	...
	int i, j, a, b, N0 = ls->N;
	int* solution0_best = (int*)((char*)malloc(buf_size));
	//int* solution0_best[buf_size]; // (int*)buf;
...
//	int* back_ptrs = (int*)buf;
	int* back_ptrs = (int*)((char*)malloc(buf_size));
...
{
	int i;
	void* array = malloc((N0+N1)*sizeof(Node) + A_max*sizeof(Assignment) + E_max*sizeof(Edge) + N0*sizeof(int) + N0*sizeof(float) );
	memset(array, 0, (N0+N1)*sizeof(Node) + A_max*sizeof(Assignment) + E_max*sizeof(Edge) + N0*sizeof(int) + N0*sizeof(float) );
	nodes[0] = (Node*) array;
	nodes[1] = nodes[0] + N0;
	...
	solution_weights = (float*)(solution_left_best+N0);
	for (i=0; i<N0; i++) solution_weights[i] = 0;
	N[0] = N0;
	N[1] = N1;
	N_max = (N0 > N1) ? N0 : N1;
...
double GraphMatching::_UpperBoundCallbackFn(void* user_arg)
{
	...
	Assignment* a;
	for (a=m->assignments; a<m->assignments+m->A; a++) a->numSubprobs = 0;
	SubgradientDecomposition::Subproblem* s;
	for (s=m->sd->GetFirstSubproblem(); s; s=m->sd->GetNextSubproblem())
	{
		for (int k=0; k<s->var_num; k++) {
			int var=s->array[k];
			if (var<m->A) {
				a= m->assignments + var;
				a->numSubprobs +=1;
	...
	for (int n=0; n<(m->N0); n++) m->solution_weights[n] = 0;
	for (i=m->nodes[0]; i<m->nodes[0]+m->N0; i++) {
		a=i->match;
		if (a) {
			m->solution_weights[(int)(i-m->nodes[0])] = (float)a->numProposals/a->numSubprobs;
		}
	}
...
void GraphMatching::_DummyCallbackFn(void* user_arg)
{
	GraphMatching* m = (GraphMatching*)user_arg;
	for (Assignment* a=m->assignments; a<m->assignments+m->A; a++) a->numProposals=0;
}
...
double GraphMatching::SolveDD(int iter_max, double gap_threshold)
{
	...
	double res = sd->Maximize(iter_max, gap_threshold, _UpperBoundCallbackFn, _DummyCallbackFn);

	int* maxProposals = new int[N0];
	memset(maxProposals,0,N0*sizeof(int));
	for (a=assignments; a<assignments+A; a++) {
		int nodeID = (int)(a->i[0]-nodes[0]);
		if (a->numProposals > maxProposals[nodeID]) {
			maxProposals[nodeID] = a->numProposals;
			solution_left_best[nodeID] = (int)(a->i[1]-nodes[1]);
		}
	}
	delete[] maxProposals;
